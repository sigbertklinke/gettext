...
`gettext`, `G`, `ngettext` or `N` command. The second column is the message id 
and the third column contains the plural form. The fourth column contains the context (default: `NA`).
...
```{r eval=FALSE}
library("mmstat.utils")
demoApp('app1')   # Shiny app without language support 
demoApp('app2en') # Shiny app in the default language (English)
demoApp('app2de') # Shiny app in German
```
...
users will have just one app and will simply use the default domain `''`. The current language and domain can be set by:
...
of a file name the `iso639` data object is used.
